...
client.on(
  ...
  async () => {
    ...
    owner_db.fetch_owners().then(async (data) => {
      ...
      // console.log(owners);
      // owners.push(data[owner_whatsapp_no]);
    })
    ...
    cron.schedule('*/2 * * * *', async () => {
      ...
      ride_data.forEach(async (r_data) => {

        var hours = Math.abs(new Date() - r_data.registered_datetime) / 36e5;
        // console.log(hours)


        // let registered_datetime = parseFloat(moment(r_data.registered_datetime, 'hh:mm A').format('HH:mm').replace(":", "."));
        // let currentTime = parseFloat(moment().format("HH:mm").toString().replace(":", "."));

        // console.log(currentTime - registered_datetime)
        if (hours >= 1) {
          // console.log(`Status Deleted ${r_data.user_whatsapp_no} ${hours}`)
          ...
          let number_format = `${r_funcs.extract_integer(r_data.user_whatsapp_no)}@c.us`;
          await client.sendMessage(number_format, "Your Ride is Completed By BookMyDriver");
        }
      });
...
const SendAllConfirmedBookingsToOwner = async (owner_whatsapp_no, client_number = null) => {
  ...
  if (client_number != null) {
    ...
    // let customer_number_format = `${r_funcs.extract_integer(client_number)}@c.us`;
...
client.on('message', async (msg) => {
    ...
    if (owner_number != -1) {
      ...
      if (chat_init_msg.includes(message)) {
        ...
        if (customer_number.length != 12) {
          await client.sendMessage(number_format, "Please Enter Valid Number with Country Code (91) in Prefix");
        }
        else if (command[0] == "showbooking" || command[0] == "showbookings") {
          await SendAllConfirmedBookingsToOwner(number_format, `+${customer_number}`);
        }
        else if (command[0] == "complete") {
          let ride_data = await user_ride_db.fetch_confimed_client_ride(`+${customer_number}`);
          console.log(ride_data);
          if (ride_data.length > 0) {
            ...
            if (ride_data[0].ride_status == 2) {
              ...
              await client.sendMessage(number_format, "Customer Ride Status Updated From *Pending* to *Completed* !");
              ...
              await client.sendMessage(number_format, "Ride Already Completed !");
            }
      ...
      await user_db.isUserExists(user_whatsapp_no).then(async (result) => {
        if (result.length < 1) {
          ...
          if (result[0].user_fullname === null) {
            ...
            // console.log(user_ride_data.length , user_ride_data);
      ...
      await user_db.isUserExists(user_whatsapp_no).then(async (result) => {
        if (result.length == 1) {
          ...
          if (msg.body.toLowerCase() == 'cancel') {
            ...
            if (ride_data[0].ride_status == 1 || ride_data[0].ride_status == 2) {
            ...
            if (ride_data.length > 0) {
              ...
              let booked_ride__data_msg = `${user_data.user_fullname
          ...
          // ask for day or night and save
            ...
            if (moment(msg.body, 'DD/MM/YYYY', true).isValid()) {
              ...
              if (r_funcs.isToday(moment(db_format_date).toDate()) || r_funcs.isFuture(moment(db_format_date).toDate())) {
            ...
            if (moment(msg.body, 'LT', true).isValid()) {
              if (msg.body.toLowerCase().endsWith("am") || msg.body.toLowerCase().endsWith("pm")) {
                let ride_data = await user_ride_db.get_user_ride(user_whatsapp_no);

                if (r_funcs.isToday(ride_data[0].ride_date)) {
                  let convertedTime = parseFloat(moment(msg.body.toUpperCase(), 'hh:mm A').format('HH:mm').replace(":", "."));
                  let currentTime = parseFloat(moment().format("HH:mm").toString().replace(":", "."));

                  if (convertedTime > currentTime) {
                    await user_ride_db.set_ride_time(user_whatsapp_no, msg.body.toUpperCase());
                    await user_db.updateUserState(user_whatsapp_no, 9);
                    client.sendMessage(msg.from, user_state.user_state_9());
                  } else {
                    client.sendMessage(msg.from, "The time Should be More than Present Time!");
                  }
  ...
  // }
  // else if (msg.body.search("!name") != -1) {
  //     await msg.reply("hi " + msg.body.replace("!name", ""));
  // }
  // else if (msg.body == "!hi" || msg.body == "!hello" || msg.body == "!hey") {
  //     client.sendMessage(msg.from, "Hello Sir");
  //     client.sendMessage(msg.from, "How can i help you?");
  //     client.sendMessage(msg.from, "Options : 1 - login");
  //     client.sendMessage(msg.from, "Options : 2 - Register");
  // }
  // else if(msg.body == "1" || msg.body == "2"){
  //     client.sendMessage(msg.from, "Under Development");
  // }
  ...
  //         client.sendMessage(msg.from, response.data.value);
  //     } catch (error) {
  //         console.error(error);
  //     }
  // }
